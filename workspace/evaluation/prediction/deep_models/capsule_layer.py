...
class Mask(layers.Layer):
    ...
    def compute_output_shape(self, input_shape):
        if type(input_shape[0]) is tuple: return tuple([None, input_shape[0][1] * input_shape[0][2]]) # true label provided
        else                            : return tuple([None, input_shape[1]    * input_shape[2]]) # no true label provided
...
def PrimaryCap(inputs, dim_capsule, n_channels, kernel_size, strides, padding = 'valid'):
    """
    Apply Conv1D `n_channels` times and concatenate all capsules
    :param inputs: 3D tensor, shape=[None, timesteps, channels]
